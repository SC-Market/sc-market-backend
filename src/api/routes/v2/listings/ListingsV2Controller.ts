    ...
    try {
      ...
      let query = db("listing_search as ls")
        .select(
          ...
          "ls.quality_tier_max",
          db.raw(`(
            SELECT MIN((iv.attributes->>'quality_value')::int)
          ...
          db.raw(`(SELECT COALESCE(sh.badge_ids, '{}') FROM shops sh WHERE sh.shop_id = ls.shop_id) AS shop_badge_ids`),
          "ls.updated_at",
          "ls.game_item_id",
          "ls.game_item_name",
          "ls.game_item_type",
      ...
      const listings: ListingSearchResult[] = results.map((row: any) => ({
        ...
        shop_badge_ids: row.shop_badge_ids || [],
