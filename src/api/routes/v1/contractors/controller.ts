...
export const post_auth_link: RequestHandler = async (req, res) => {
  ...
  try {
    ...
    if (cobj) {
      res
        .status(409)
        .json(createConflictErrorResponse("Org is already registered!"))
      return
    }
  } catch {}
...
export const delete_spectrum_id: RequestHandler = async (req, res) => {
  ...
  if (!contractor) {
    res.status(404).json(createNotFoundErrorResponse("Organization not found"))
    return
  }
...
export const post_spectrum_id_accept: RequestHandler = async (
  ...
  try {
    ...
    if (contractor.archived) {
      res
        .status(409)
        .json(createConflictErrorResponse("Organization has been archived"))
      return
    }
