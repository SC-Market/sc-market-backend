...
export async function up(knex: Knex): Promise<void> {
  ...
  // Index on listing_item_lots for efficient quantity computation
  ...
  await knex.raw(`
    COMMENT ON INDEX idx_listing_item_lots_item_listed IS 
...
export async function down(knex: Knex): Promise<void> {
  ...
  await knex.schema.alterTable("listing_item_lots", (table) => {
    table.dropIndex(["item_id", "listed"], "idx_listing_item_lots_item_listed")
  })
}
