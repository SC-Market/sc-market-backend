...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw(`
    ...
    SELECT
      ...
      (SELECT MIN((iv.attributes->>'quality_value')::numeric) FROM listing_item_lots sl JOIN item_variants iv ON sl.variant_id = iv.variant_id WHERE sl.item_id = li.item_id AND sl.listed = true) AS quality_value_min,
      (SELECT MAX((iv.attributes->>'quality_value')::numeric) FROM listing_item_lots sl JOIN item_variants iv ON sl.variant_id = iv.variant_id WHERE sl.item_id = li.item_id AND sl.listed = true) AS quality_value_max,
