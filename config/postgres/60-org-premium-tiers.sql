...
CREATE TABLE IF NOT EXISTS public.org_premium_tiers (
    ...
    contractor_id UUID NOT NULL REFERENCES public.contractors(contractor_id) ON DELETE CASCADE,
    tier VARCHAR NOT NULL DEFAULT 'white_label',
    custom_domain VARCHAR UNIQUE,
    granted_by UUID NOT NULL REFERENCES public.accounts(user_id),
...
CREATE TABLE IF NOT EXISTS public.org_themes (
    ...
    updated_by UUID NOT NULL REFERENCES public.accounts(user_id),
